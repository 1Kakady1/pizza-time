...
const loginRequest= (state:IUser, { payload }: { payload: IUserActionProps })=>{
  state.isLoad = true;
  state.email=payload.email;
  state.password = payload.password;
}

const loginRequestSuccess= (state:IUser, { payload }: { payload: IUser | {} })=>{
  return {
    ...state,
    isLoad: false,
    isAuth: true,
...
const getUserLaodingRequest= (state:IUser, { payload }: { payload: {userID: string, email: string | null} })=>{
  state.isLoad = true;
  state.isAuth = false;
...
const getUserRequestSuccess= (state:IUser,{payload}:{payload: IUser}) =>{
    return{
      ...state,
      isLoad: false,
      isAuth: true,
...
const getUserRequestFailed= (state:IUser, {payload}:  { payload: string }) =>{
  state.isLoad = false;
  state.isAuth = false;
  state.error = payload;
}
...
const logoutRequestFailed = (state:IUser, {payload}:  { payload: string })=>{
  state.error = payload;
}
