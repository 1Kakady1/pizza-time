...
import { Animated,ImageBackground, Text, View,Image,Alert, BackHandler, TextInput as RNTextInput, Keyboard, TouchableWithoutFeedback } from 'react-native';
...
export const Auth = () => {
    ...
    React.useEffect( ()=>{
        fb.firebase.auth().onAuthStateChanged(function(user) {
            if (user && !isLoading && !isAuth) {
              dispatch(toUserAction.getUserActionLaodingRequest({userID: user.uid, email: user.email}));
            } 
        });
    ...
    const setInput = ({type, value ,errorList, clear=false}:{type: string,value:string, errorList?: IError, clear?:boolean})=>{

        if(!clear && errorList){
            switch(type){
                case 'email':
                    setTextEmail((prevState:any) => {
                        return {...prevState, value, valid: errorList.valid,keyError: errorList.key, error: t(errorList.key),init:true};
                    });break;
                case 'password':
            ...
            Animated.timing(fadeAnim[type],{
                ...
                switch(type){
                    case 'email':
                        setTextEmail((prevState:any) => {
                            return {...prevState, valid: true, keyError:"", error:'',init:false};
                        }); break;
                    case 'password':
                        setTextPassword((prevState:any) => {
                            return {...prevState, valid: true,keyError:"", error:'',init:false};
    ...
    const  handlerInput = (type:string, text:string):void =>{
        ...
        if(inputs[type].init === true){
            ...
            const errorsList = validator.check_valid_input(inputs[type].validation,inputs[type].value);
           
            for(let i = 0; i < errorsList.length;i++){
                const item = errorsList[i];
                
                if(item.valid === false){ // error true
             ...
             if(flagClearError === true){ // clear error
    ...
    const handlerForm = () =>{
        ...
        if(textEmail.valid && textPassword.valid){
            dispatch(toUserAction.loginActionRequest({email: textEmail.value, password: textPassword.value}))
        } else {
            Alert.alert(
                "",
                t("msg.modal.error"),
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={styles.container} >
                    ...
                    <View style={styles.wrapperChildren}>
                        <View style={[styles.inputContainer,{alignItems:"center",paddingBottom:4}]}>
                            <Image style={{width: 120,height: 120}} source={require("../../../assets/img/logo.png")}></Image>
                        </View>
                        <View style={styles.inputContainer}>
                            <TextInput
                                ...
                                error={textEmail.init === true? !textEmail.valid : false}
                                style={styles.input}
                                onChangeText={text => {
                                    setTextEmail({...textEmail,value: text.trim()})
                                    handlerInput("email",text.trim())
                                }}

                            />
                            <Animated.View style={[styles.error,{transform:[
                                {translateX: fadeAnim.email}
                            ]}]} >
                        ...
                        <View style={styles.inputContainer}>
                            <TextInput
                                ...
                                error={textPassword.init === true? !textPassword.valid : false}
                                style={styles.input}
                                secureTextEntry={!isShowPassword}
                                onChangeText={text => {
                                    setTextPassword({...textPassword,value: text})
                                    handlerInput("password",text)
                                }}

                            />
                            <Button style={styles.showPassword} onPress={()=> setShowPassword(!isShowPassword)}>
                                <Icon color={primary[1]} name={!isShowPassword ? "eye" : "eye-slash"} size={24}  />
                            </Button>
                            <Animated.View style={[styles.error,{transform:[
                        ...
                        <View style={styles.inputContainer}>
                            <Button icon="login" loading={isLoading} mode="contained" contentStyle={styles.btnContainer} onPress={handlerForm}>
                            {t("sing-in")}
                            </Button>
                        </View>
                    </View>
