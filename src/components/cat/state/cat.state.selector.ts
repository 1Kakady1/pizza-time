...
export const categorySelector = createFeatureSelector<ICatData>(
    CAT_KEY 
);
...
const getCat =  (id: string) => createSelector(
    categorySelector,
    ({ cat }) => cat.find((x) => x.id === id)
);
...
export const toCategorySelector = {category, isLoad, getCat, categorySelector, currentCat};