...
export const SearchItem = ({title, price, preview, id, index, size, onClose}: ISearchItemProps) => {
    ...
    const nav = ()=> navigation.navigate(SCREENS_STACK.modal, {
        ...
        <TouchableOpacity style={styles.wrapper} onPress={()=> {  onClose(); nav(); }}>
            <Animatable.View animation={"fadeIn"} delay={200 * index} style={styles.container}>
                ...
                <View style={styles.colRight}>
                    ...
                    <Text style={styles.price}> {t("search.price", {size: size[0]})} {priceCurrent}$</Text>
                    <View style={styles.info}>
                        <Icon style={styles.icon}  color={text.title} name={"arrow-top-left-bottom-right-bold"} size={20} />
                        {
                            size.map((item:string)=> <Text style={styles.size} key={item}>{item.toUpperCase()}</Text>)
                        }
                    </View>
                </View>
