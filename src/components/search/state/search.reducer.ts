...
const setSearchResult = (state:ISearch,{payload}: {payload: ISearchResult[]})=>{
...
const searchRequestSuccess= (state:ISearch, { payload }: { payload: ISearchResult[]})=>{
  return {
    ...state,
    isLoad: false,
    error: "",
...
const searchRequestFailed= (state:ISearch, { payload }: { payload: string })=>{
  state.isLoad = false;
  state.error = payload;
}
