...
export const CartPanelContainer = ({item}:{item: ICartItemProps}) =>{
    ...
    const onSub = (item: ICartItem) => dispatch(toCartAction.sub({prod: item, size}));
    const onSize = (size: string) => dispatch(toCartPanelAction.changeSize(size));
    ...
    return <CartPanel item={item} size={size} onAdd={onAdd} onSub={onSub} onSize={onSize}/>
}