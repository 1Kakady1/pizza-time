...
export const ModalWindow = ({btn,children,btnProps}: IModalProps) => {
  ...
  const showModal = () => {console.log("show");setVisible(true)};
  ...
  return (
    <View>
      ...
      {
        btn({onPress: showModal, label: btnProps?.label || "", icon: btnProps?.icon, small: btnProps?.small, onLongPress: btnProps?.onLongPress })
      }
    </View>
  );
};