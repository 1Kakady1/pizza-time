...
export  const  RenderItem = ()=>{
    ...
    return <View style={[{height: 124},styles.itemPlaceholder]}>
        <SkeletonPlaceholder speed={1700} backgroundColor={gray[100]} highlightColor={gray[50]}>
...
export const CardHorizontalPlaceholder = ({count}:ICardHorizontalPlaceholder) => {
    const items = dataCount(count).map((item:string)=><RenderItem  key={item} ></RenderItem>)
    return  <>{items}</>
};