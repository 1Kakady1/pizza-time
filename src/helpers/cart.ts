...
export const getPrice = (price: price, filtersSize:string[], size:string="") =>{
    ...
    const index = findSizeIndex !== -1 ? filtersSize[findSizeIndex] : filtersSize[0];

    return price[index]
}

export const getProdCounter = (post: IPostItem | undefined, cart:ICartItem[] ,id: string) =>{
    ...
    if(post){
        ...
        for(let i = 0; i < size.length; i++){
            counter[size[i]] = cart.find(x => x.productSize === size[i] && x.id === id)?.count || 0;
...
export const getCounterCounter = (cart:ICartItem[], count:isFullCount = "productCount"): number | IGetCounterCounter =>{
